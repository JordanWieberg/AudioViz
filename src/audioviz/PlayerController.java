...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        //Sets the length number display with the first decimal place
        String ms = df.format(duration.toMillis());
        lengthText.setText(ms);
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        //Sets the current number display with the first decimal place
        String ms = df.format(ct.toMillis());
        currentText.setText(ms);
        timeSlider.setValue(Double.parseDouble(ms));
    ...
    //Allows the user to move the slider and the audio updates
    @FXML
    private void sliderUpdate(Event event) {
        if (mediaPlayer != null) {
    ...
    //Pauses the audio while the user is choosing where the slider is going to land
    @FXML
    private void sliderPaused(Event event) {
        if (mediaPlayer != null) {
           mediaPlayer.pause(); 
        }  
    }
